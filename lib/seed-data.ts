...
export async function setupDatabase() {
  try {
    ...
    const { error: brandsError } = await supabase.rpc("create_brands_table_if_not_exists")
    if (brandsError) throw brandsError

    const { error: collectionsError } = await supabase.rpc("create_collections_table_if_not_exists")
    if (collectionsError) throw collectionsError

    const { error: sellersError } = await supabase.rpc("create_sellers_table_if_not_exists")
    if (sellersError) throw sellersError

    const { error: productsError } = await supabase.rpc("create_products_table_if_not_exists")
    if (productsError) throw productsError

    const { error: requestsError } = await supabase.rpc("create_product_requests_table_if_not_exists")
    if (requestsError) throw requestsError

    // Seed the data
    const { error: seedBrandsError } = await supabase.rpc("seed_brands")
    if (seedBrandsError) throw seedBrandsError

    const { error: seedCollectionsError } = await supabase.rpc("seed_collections")
    if (seedCollectionsError) throw seedCollectionsError

    const { error: seedSellersError } = await supabase.rpc("seed_sellers")
    if (seedSellersError) throw seedSellersError

    const { error: seedProductsError } = await supabase.rpc("seed_products")
    if (seedProductsError) throw seedProductsError

    const { error: seedRequestsError } = await supabase.rpc("seed_product_requests")
    if (seedRequestsError) throw seedRequestsError
